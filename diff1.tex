...
%DIF DEL dimmolv0.tex   Sat Apr 13 09:26:30 2019
%DIF ADD dimmol.tex     Sat Apr 13 11:32:52 2019
\usepackage{amsmath,amssymb}
\usepackage{graphicx}
\usepackage{authblk}
%DIF 5d5
%DIF < % \usepackage[square,sort,comma,numbers]{natbib}
%DIF -------
\usepackage{float}
\usepackage{xcolor}
...
%DIF <  \affil[2]{Departamento de F\'isica, Facultad de Ciencias Exactas y Naturales, 
%DIF <  Universidad de Buenos Aires, Buenos Aires, Argentina}
...
By examination of several inelastic processes \DIFaddbegin \DIFadd{in the first Born 
approximation}\DIFaddend , we demonstrate the inconveniences of this \DIFdelbegin \DIFdel{approximation}\DIFdelend \DIFaddbegin \DIFadd{approach}\DIFaddend . 
Instead, we prescribe the usage of effective potentials obtained with 
the \DIFdelbegin \DIFdel{Depurated Inversion 
Method }\DIFdelend \DIFaddbegin \DIFadd{depurated inversion method }\DIFaddend (DIM), which is described in this work. 
We also extended this method for molecular systems. Calculations of 
single \DIFaddbegin \DIFadd{first--order }\DIFaddend photoionisation and proton--impact ionisation with 
...
simpler way }\DIFdelend \DIFaddbegin \DIFadd{generating more 
directly }\DIFaddend the orthogonal wavefunctions for the interacting particles.
This \DIFdelbegin \DIFdel{should include detailed }\DIFdelend \DIFaddbegin \DIFadd{approach should include individual }\DIFaddend $nl$--orbital potentials, a 
...
\DIFdelbegin \DIFdel{However, this is not a simple task.
}\DIFdelend 
...
approximation \DIFaddbegin \DIFadd{(PPA)}\DIFaddend , in which all the complexity of the wavefunctions near 
...
such as the {\sc parsec} for example\DIFaddbegin \DIFadd{~}\DIFaddend \cite{parsecwebpage,Chelikowsky1994}, 
...
Thus, if \DIFdelbegin \DIFdel{this approach is }\DIFdelend \DIFaddbegin \DIFadd{PPA were }\DIFaddend applicable in the field of collisions theory, 
one would save \DIFaddbegin \DIFadd{an }\DIFaddend enormous amount of computational resources.
\DIFdelbegin \DIFdel{In this article, we explore the possibility of using pseudopotentials 
...
\DIFdelbegin \DIFdel{In previous works \mbox{%DIFAUXCMD
\cite{Mendez2015,Mendez2016,Mendez2018}}\hspace{0pt}%DIFAUXCMD
, we developed the Depurated Inversion Method 
}\DIFdelend \DIFaddbegin \DIFadd{Another interesting approach is the depurated inversion method 
}\DIFaddend (DIM) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Mendez2015,Mendez2016,Mendez2018}}\hspace{0pt}%DIFAUXCMD
, which allows
obtaining accurate}\DIFaddend , \DIFdelbegin \DIFdel{allowing to obtain accurate}\DIFdelend effective potentials by substituting the coupled 
multielectron equations into a Kohn--Sham type equation. 
In \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend first step, the \DIFdelbegin \DIFdel{corresponding effective }\DIFdelend potential is obtained through inversion of \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the 
one--electron }\DIFaddend equation. Next, \DIFdelbegin \DIFdel{a }\DIFdelend careful optimisation of \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend potential is 
carried on, eliminating poles, and imposing \DIFdelbegin \DIFdel{analytically, }\DIFdelend the appropriate boundary 
conditions \DIFaddbegin \DIFadd{analytically}\DIFaddend . In that way, the DIM potentials are \DIFdelbegin \DIFdel{parametrized }\DIFdelend \DIFaddbegin \DIFadd{parametrised 
}\DIFaddend in simple analytical expressions.
\DIFaddbegin 

\DIFaddend In the present work, we \DIFdelbegin \DIFdel{show how these DIM potentials overcome the deficiencies of the pseudopotentials, calculating the }\DIFdelend \DIFaddbegin \DIFadd{explore the possibility of implementing an 
...
A wide variety of }{\it \DIFadd{ab initio}} \DIFadd{methods have been 
implemented to compute the scattering }\DIFaddend cross sections for \DIFdelbegin \DIFdel{inelastic problems}\DIFdelend \DIFaddbegin \DIFadd{atomic targets, 
from the early implementations of the first Born 
approximation (FBA)~\mbox{%DIFAUXCMD
\cite{BatesBook,McDowell1961}}\hspace{0pt}%DIFAUXCMD
, to the much 
more sophisticated time--dependent close--coupling 
methods~\mbox{%DIFAUXCMD
\cite{LeePindzola2011,Pindzola2016}}\hspace{0pt}%DIFAUXCMD
...
high energy limit}\DIFaddend .

\DIFdelbegin \DIFdel{Finally, }\DIFdelend \DIFaddbegin \DIFadd{In Section~\ref{sec:colproc}, we examine the above mentioned 
...
see~\mbox{%DIFAUXCMD
\cite{Kirchner1998,Fiori2001}}\hspace{0pt}%DIFAUXCMD
).
...
\DIFadd{On the other hand, the description of molecular systems constitutes 
a real challenge due to their nonspherical symmetry and multicenter 
character. Many }\textit{\DIFadd{ab initio}} \DIFadd{and semi--empirical theoretical 
approximations \mbox{%DIFAUXCMD
\cite{Szabo1996,Helgaker2000,Schaefer2004} }\hspace{0pt}%DIFAUXCMD
have been 
developed to this end over the last century. 
In this work, we present }\DIFaddend an extension of the DIM method for simple 
molecular \DIFdelbegin \DIFdel{system 
is developed}\DIFdelend \DIFaddbegin \DIFadd{systems}\DIFaddend , providing a new \DIFdelbegin \DIFdel{parameteric expression and results for }\DIFdelend \DIFaddbegin \DIFadd{parametric expression for the 
potentials. 
The target description is once again tested by examination of its 
performance in first--order }\DIFaddend collisional processes. 
...
\subsection{\DIFdelbegin \DIFdel{Pseudopotentials}\DIFdelend \DIFaddbegin \DIFadd{Pseudopotential Approximation}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \label{sec:PPs}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:PPAs}
\DIFaddend 

The pseudopotential \DIFdelbegin \DIFdel{(PP) method }\DIFdelend \DIFaddbegin \DIFadd{approximation }\DIFaddend consists of replacing the Coulomb 
...
In general, the pseudopotentials \DIFdelbegin \DIFdel{$V_{\mbox{\tiny PP}}$ }\DIFdelend \DIFaddbegin \DIFadd{$V_{\mbox{\tiny PPA}}$ }\DIFaddend can be defined 
through a pseudo--charge \DIFdelbegin \DIFdel{$Z_{\mbox{\tiny PP}}$ as
}\DIFdelend \DIFaddbegin \DIFadd{$Z_{\mbox{\tiny PPA}}$ as
}\DIFaddend \begin{equation}
 V\DIFdelbegin \DIFdel{_{\mbox{\tiny PP}}}\DIFdelend \DIFaddbegin \DIFadd{_{\mbox{\tiny PPA}}}\DIFaddend (r)=-\DIFdelbegin \DIFdel{\frac{Z_{\mbox{\tiny PP}}(r)}{r} }\DIFdelend \DIFaddbegin \DIFadd{\frac{Z_{\mbox{\tiny PPA}}(r)}{r} }\DIFaddend \,,
 \label{eq:pseudopotential}
\end{equation}
\begin{equation}
 Z\DIFdelbegin \DIFdel{_{\mbox{\tiny PP}}}\DIFdelend \DIFaddbegin \DIFadd{_{\mbox{\tiny PPA}}}\DIFaddend (r)=\bigg\{
 \begin{array}{cl}
  f(r), & r\leq r_c \\
  1, & r>r_c 
 \end{array} 
...
argon. \DIFaddbegin \DIFadd{Notice that the pseudopotential behaves as $-1/r$ (dot--dash 
line) in the valence region, as defined in Eqs. (\ref{eq:pseudopotential})
and (\ref{eq:pseudocharge}).
}\DIFaddend The one--electron Hartree--Fock wavefunction is included
...
implementing \DIFdelbegin \DIFdel{PPs }\DIFdelend \DIFaddbegin \DIFadd{pseudopotentials }\DIFaddend in collisional processes calculations for two 
...
matrix element through the first Born \DIFdelbegin \DIFdel{(FB)approximation}\DIFdelend \DIFaddbegin \DIFadd{approximation (FBA)}\DIFaddend , given by
\begin{equation}
 T_{\!fi}\DIFdelbegin \DIFdel{^{\mbox{\scriptsize FB}} }\DIFdelend \DIFaddbegin \DIFadd{^{\mbox{\scriptsize FBA}} }\DIFaddend = \widetilde{V}(\mathbf{p}) F_{\!fi}(\mathbf{p}) \,.
...
\DIFdelbegin %DIFDELCMD < \label{sec:photo}
%DIFDELCMD < %%%
...
The \DIFdelbegin \DIFdel{description of molecular systems constitutes a real challenge due 
to their generally multicentered and highly non--central nature. 
Many }\textit{\DIFdel{ab initio}} %DIFAUXCMD
\DIFdel{and semi--empirical theoretical approximations 
have been developed to this end over the last century.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In this work, the Depurated Inversion Method }\DIFdelend \DIFaddbegin \DIFadd{depurated inversion method described above }\DIFaddend is extended to determine 
effective potentials for molecules. \DIFdelbegin \DIFdel{In this Section, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend theoretical methods are 
\DIFdelbegin \DIFdel{stablished }\DIFdelend \DIFaddbegin \DIFadd{established in this Section, }\DIFaddend and results for methane are given. 
Furthermore, \DIFdelbegin \DIFdel{collisional processes are computed with these molecular DIM potentials}\DIFdelend \DIFaddbegin \DIFadd{the molecular description of CH$_4$ given by DIM is 
tested by computing two collisional processes within the FBA}\DIFaddend .
...
we considered \DIFaddbegin \DIFadd{CH$_4$, which is a highly symmetric 
molecule. Furthermore, the spherical symmetry of }\DIFaddend the methane molecule
\DIFdelbegin \DIFdel{, which has a suitable
spherical geometry}\DIFdelend \DIFaddbegin \DIFadd{allows the system to be successfully described, for high energies, 
with a angular averaged potential \mbox{%DIFAUXCMD
\cite{Granados2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
We computed the HF molecular orbitals and energies of CH$_4$ employing 
the UGBS basis sets of carbon and hydrogen\DIFdelbegin \DIFdel{, using the }%DIFDELCMD < {\sc %%%
\DIFdel{gamess}%DIFDELCMD < } %%%
\DIFdel{code}\DIFdelend \DIFaddbegin \DIFadd{. Previous works 
\mbox{%DIFAUXCMD
\cite{Rothenberg1971,Hariharan1972} }\hspace{0pt}%DIFAUXCMD
have shown that accurate molecular 
...
profiles and the molecular orbitals on the same footing}\DIFaddend . 
...
\DIFdelbegin %DIFDELCMD < \label{subsubsec:photoionisation}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{subsubsec:photoch4}
\DIFaddend 
...
Hoggan, P. E., Ed.; 
}\DIFaddend Advances in Quantum Chemistry\DIFaddbegin \DIFadd{;
Academic Press, 2018;
Vol.~76; pp~117--132.
}
...
In }{\it \DIFadd{At. Mol. Process.}}\DIFadd{;
Bates, D. R., Ed;
...
\DIFadd{McDowell, M. R. C.; Peach, G.
Ionization of Lithium by Fast Protons and Electrons.
}{\it \DIFadd{Phys. Rev.}} \DIFaddend {\bf \DIFdelbegin \DIFdel{76}\DIFdelend \DIFaddbegin \DIFadd{1961}\DIFaddend }, \DIFdelbegin \DIFdel{117--132 (2018). }\DIFdelend \DIFaddbegin \DIFadd{121, 1383--1387.
}

\bibitem{LeePindzola2011}
\DIFadd{Lee, T.-G.; Pindzola, M. S.
Proton-impact excitation of lithium using a time--dependent 
close--coupling method.
}{\it \DIFadd{Phys. Rev. A}} {\bf \DIFadd{2011}}\DIFadd{, 84, 052712.
}

\bibitem{Pindzola2016}
\DIFadd{Pindzola, M. S.; Colgan, J.; Robicheaux, F.; Lee, T.-G.; Ciappina, M. F.;
Foster, M.; Ludlow, J. A.; Abdel-Naby,
 S. A.
Time-Dependent Close--Coupling Calculations for Ion--Impact Ionization 
of Atoms and Molecules. 
In }{\it \DIFadd{Advances In Atomic, Molecular, and Optical Physics}}\DIFadd{;
...
\DIFadd{Kirchner, T.; Guly\'as, L.; L\"udde, H. J.; Engel, E.; Dreizler, R. M. 
Influence of electronic exchange on single and multiple processes in 
collisions between bare ions and noble--gas atoms.
}{\it \DIFadd{Phys. Rev. A}} {\bf \DIFadd{1998}}\DIFadd{, 58, 2063--2076.
}

\bibitem{Fiori2001}
\DIFadd{Fiori, M. R.; Jalbert, G.; Bielschowsky; C. E.; Cravero, W.
Ionization of lithium by impact of fast ions.
}{\it \DIFadd{Phys. Rev. A}} {\bf \DIFadd{2001}}\DIFadd{, 64, 012705.
}

\bibitem{Szabo1996}
\DIFadd{Szabo, A.; Ostlund, N. S.
}{\it \DIFadd{Modern Quantum Chemistry: Introduction to Advanced Electronic 
Structure Theory}}\DIFadd{,
Dover Publications, Inc.: Mineola, New York, 1996.
}

\bibitem{Helgaker2000}
\DIFadd{Helgaker, T.; J}{\DIFadd{\o}}\DIFadd{rgensen, P.; Olsen, J.
}{\it \DIFadd{Molecular Electronic-Structure Theory}}\DIFadd{,
John Wiley }{\DIFadd{\&}} \DIFadd{Sons, Ltd: Chichester, UK, 2000.
}

\bibitem{Schaefer2004}
\DIFadd{Schaefer, H. F. III
}{\it \DIFadd{Quantum Chemistry: The Development of Ab Initio Methods in
Molecular Electronic Structure Theory}}\DIFadd{,
...
}\DIFaddend {\DIFdelbegin %DIFDELCMD < \em %%%
\DIFdel{Computational atomic structure: an MCHF approach}\DIFdelend \DIFaddbegin \it \DIFadd{Computational Atomic Structure: An MCHF Approach}\DIFaddend },
...
}\DIFdelend {\DIFdelbegin %DIFDELCMD < \em %%%
\DIFdel{Atomic structure theory : lectures on atomic physics}\DIFdelend \DIFaddbegin \it \DIFadd{Atomic Structure Theory: Lectures on Atomic Physics}\DIFaddend },
...
\DIFadd{Rothenberg, S.; Schaefer, H. F.
Methane as a Numerical Experiment for Polarization Basis Function Selection.
}{\it \DIFadd{J. Chem. Phys.}} {\bf \DIFadd{1971}}\DIFadd{, 54}\DIFaddend , \DIFaddbegin \DIFadd{2764--2766.
}

\bibitem{Hariharan1972}
\DIFadd{Hariharan, P. C.; Pople, J. A.
The effect of d-functions on molecular orbital energies for hydrocarbons.
}{\it \DIFadd{Chem. Phys. Lett.}} \DIFaddend {\bf \DIFdelbegin \DIFdel{41}\DIFdelend \DIFaddbegin \DIFadd{1972}\DIFaddend }, \DIFdelbegin \DIFdel{1167--1189 (2005)}\DIFdelend \DIFaddbegin \DIFadd{16, 217--219}\DIFaddend .
