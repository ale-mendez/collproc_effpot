...
%DIF DEL dimmolv2.tex   Mon Apr 29 17:02:24 2019
%DIF ADD dimmol.tex     Mon Apr 29 21:02:29 2019
...
Instead, we \DIFdelbegin \DIFdel{prescribe the usage }\DIFdelend \DIFaddbegin \DIFadd{advocate use }\DIFaddend of effective potentials obtained with 
the depurated inversion method (DIM)\DIFdelbegin \DIFdel{, which is described in this work. We also extended this method for }\DIFdelend \DIFaddbegin \DIFadd{. In this contribution, we 
extend this method to }\DIFaddend molecular systems. Calculations of 
single first--order photoionisation and proton--impact ionisation with 
DIM show good \DIFdelbegin \DIFdel{agreements }\DIFdelend \DIFaddbegin \DIFadd{agreement }\DIFaddend with experimental results \DIFaddbegin \DIFadd{for both atoms and
molecules}\DIFaddend .
\end{abstract}
...
potential accounting for these states would allow \DIFdelbegin \DIFdel{generating more directly }\DIFdelend \DIFaddbegin \DIFadd{more direct
generation of }\DIFaddend the orthogonal wavefunctions for the interacting 
...
In this context, one \DIFdelbegin \DIFdel{bright }\DIFdelend \DIFaddbegin \DIFadd{promising }\DIFaddend idea emerges from the pseudopotential
...
example~\cite{parsecwebpage,Chelikowsky1994}, permit \DIFdelbegin \DIFdel{to use }\DIFdelend \DIFaddbegin \DIFadd{the use of }\DIFaddend an 
...
PPA were applicable in the field of \DIFdelbegin \DIFdel{collisions }\DIFdelend \DIFaddbegin \DIFadd{collision }\DIFaddend theory, one would save 
an enormous amount of computational resources.

Another interesting approach is the depurated inversion method 
(DIM) \cite{Mendez2015,Mendez2016,Mendez2018}, which allows \DIFdelbegin \DIFdel{obtaining }\DIFdelend accurate, 
effective potentials \DIFaddbegin \DIFadd{to be obtained }\DIFaddend by substituting the coupled 
...
is carried \DIFdelbegin \DIFdel{on}\DIFdelend \DIFaddbegin \DIFadd{out}\DIFaddend , eliminating poles, and imposing the appropriate 
...
describe inelastic processes. \DIFdelbegin \DIFdel{Mainly}\DIFdelend \DIFaddbegin \DIFadd{In particular}\DIFaddend , we examine several 
...
end, several simplifications \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend made: (1) The calculations are 
constrained to Hamiltonians describing only the moving projectile, 
the target and the active electron\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend (2) The transition matrix 
elements are only considered in first perturbative order. If the 
first order fails, it would not \DIFdelbegin \DIFdel{have 
}\DIFdelend \DIFaddbegin \DIFadd{make }\DIFaddend any sense to extend the 
calculation to higher terms of the series. A wide variety of {\it ab 
initio} methods have been implemented to compute \DIFdelbegin \DIFdel{the }\DIFdelend scattering cross 
sections for atomic targets, from the early implementations of the 
first Born approximation (FBA)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{BatesBook,McDowell1961}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bates1962,McDowell1961}
}%DIFAUXCMD
}\DIFaddend , to more 
sophisticated fully quantum mechanical methods\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{, e.g., 
}\DIFaddend \cite{Pindzola2007,Burke2011,Bray2017,Pindzola2016}. For simplicity, 
...
\DIFdelbegin \DIFdel{In Section~\ref{sec:colproc}, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend examine the above mentioned inelastic processes for two atoms with 
...
\DIFdelbegin \DIFdel{Finally, our first--order DIM }\DIFdelend \DIFaddbegin \DIFadd{The DIM approach is further tested in the case of many--electron 
atoms by comparing }\DIFaddend photoionisation cross sections \DIFdelbegin \DIFdel{for 
many--electron atoms are compared }\DIFdelend with experimental 
measurements. 
...
first--order collisional processes\DIFdelbegin \DIFdel{. 
An implementation example is given for }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend the methane molecule \DIFaddbegin \DIFadd{being 
taken as an example.
}

\DIFadd{Whether for atoms or molecules, we shall present cross sections and 
...
The pseudopotential approximation consists \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend replacing the Coulomb 
...
Eqs.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:pseudopotential}) and (\ref{eq:pseudocharge}).
The \DIFdelbegin \DIFdel{one--electron Hartree--Fock wavefunction is included
with a dashed line. The }\DIFdelend pseudo--wavefunction agrees with the \DIFdelbegin \DIFdel{HF orbital only }\DIFdelend \DIFaddbegin \DIFadd{one--electron Hartree--Fock
(HF) orbital (dashed line) }\DIFaddend in the outer region, losing all 
...
\hspace{0.2cm}
 ...
 \caption{Pseudo and DIM bound state wavefunction \DIFaddbeginFL \DIFaddFL{for the $2s$ 
 orbital of lithium }\DIFaddendFL in (a) spatial and (b) momentum representation\DIFdelbeginFL \DIFdelFL{for the
 $2s$ orbital of lithium}\DIFdelendFL .}
 \label{fig:wavesLi}
\end{figure}
...
Fischer\DIFaddbegin \DIFadd{~}\DIFaddend \cite{FroeseFischer1997} and the {\sc nrhf} code by 
W. Johnson\DIFaddbegin \DIFadd{~}\DIFaddend \cite{Johnson2007}. The computation of Eq.~(\ref{eq:V}) 
...
The \DIFdelbegin \DIFdel{Depuration }\DIFdelend \DIFaddbegin \DIFadd{depuration }\DIFaddend method is implemented to tackle these unphysical 
...
\begin{equation}
 ...
 \label{eq:atomiczDIM}
\end{equation} 
where \mbox{$\sum z_j = Z_N-1$} \DIFdelbegin \DIFdel{. Afterwards, the parameters }\DIFdelend \DIFaddbegin \DIFadd{($Z_N$ here stands for the nuclear 
charge). The parameters $\alpha_j$ and $\beta_j$ }\DIFaddend are optimised to 
...
matrix element through the \DIFdelbegin \DIFdel{first Born approximation (FBA)}\DIFdelend \DIFaddbegin \DIFadd{FBA}\DIFaddend , given by
\begin{equation}
...
\DIFaddbegin \DIFadd{A more comprehensive formulation of the FBA can be found, for 
instance, in Ref.~\mbox{%DIFAUXCMD
\cite{McDowell1970}
}%DIFAUXCMD
. 
}\DIFaddend 
...
The pseudopotential results for the \DIFdelbegin \DIFdel{$n=2$ and 
$n=3$ }\DIFdelend \DIFaddbegin \DIFadd{$f_1=2s,\,2p$ and 
$f_2=3s,\,3p,\,3d$ }\DIFaddend final states of hydrogen agree excellently with 
...
transfer vector is large (\ref{eq:pmin}). As discussed earlier, \DIFaddbegin \DIFadd{in 
this region }\DIFaddend the bound momentum orbital 
$\widetilde{\psi}(\mathbf{k}+\mathbf{p})$ is not described adequately 
by the pseudopotentials\DIFdelbegin \DIFdel{at this region}\DIFdelend . An alternative expression for the form 
...
Fig.~\ref{fig:wavesLi}b) and hence their failure \DIFaddbegin \DIFadd{when used in the 
cross section calculation}\DIFaddend . 
...
\centering
 ...
 section \DIFaddbeginFL \DIFaddFL{from the ground state }\DIFaddendFL for hydrogen and lithium at 
 \mbox{$v_p=1$\, a.u..}}
 \label{fig:ionHLi}
\end{figure}
...
for a minimal range of velocities around $v\simeq0.77$}%DIFDELCMD < \,%%%
\DIFdel{a.u..
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  the cross sections are erroneously calculated 
%DIF >  with pseudopotentials. The results disagree completely for most of 
%DIF >  the energy values, except for a minimal range of velocities around 
%DIF >  $v\simeq0.77$\,a.u..
...
\centering
 ...
 and lithium \DIFaddbeginFL \DIFaddFL{in the ground state}\DIFaddendFL .}
 \label{fig:captureH}
\end{figure}
...
The depurated inversion method described above is extended \DIFaddbegin \DIFadd{here }\DIFaddend to 
determine effective potentials for molecules\DIFdelbegin \DIFdel{. The theoretical methods are 
established in this Section, and results for methane are given}\DIFdelend \DIFaddbegin \DIFadd{; methane is taken as an
example}\DIFaddend . Furthermore, the molecular description of CH$_4$ given by 
...
\begin{equation}
 ...
 + \sum_{i=1}^N V_H(r_i)
 + \sum\DIFdelbegin \DIFdel{_{i<1}}\DIFdelend \DIFaddbegin \DIFadd{_{i<j}}\DIFaddend ^N \frac{1}{r_{ij}}\,,
\end{equation}
\begin{equation}
...
\DIFaddbegin \DIFadd{in }\DIFaddend Eq.~(\ref{eq:centralfield-wave}) considering the single--centre
...
prominent oscillations \DIFdelbegin \DIFdel{are originated from imperceptible }\DIFdelend \DIFaddbegin \DIFadd{originate from }\DIFaddend undulations present in the MOs 
...
features~\cite{Schipper1997,Gaiduk2013}. \DIFaddbegin \DIFadd{In some cases, the 
oscillations are huge, e.g. near an electronegative atom like Cl. 
}\DIFaddend The appearance of these oscillations in the inverted potentials 
...
basis set with {\sc gamess} code~\cite{Schmidt1993,Gordon2005} \DIFdelbegin \DIFdel{.
We obtain }\DIFdelend \DIFaddbegin \DIFadd{and 
obtained }\DIFaddend inverted potentials by implementing Eq.~(\ref{eq:V}). 
...
$Z_{nl}^{\mbox{\scriptsize FD}}$ \DIFaddbegin \DIFadd{(solid line)}\DIFaddend , shows 
...
\DIFdelbegin \DIFdel{As shown in the figure, }\DIFdelend \DIFaddbegin \DIFadd{The oscillations pattern will vary }\DIFaddend for each basis set used in the 
calculations\DIFdelbegin \DIFdel{,
different oscillation profiles will arise, which are defined }\DIFdelend \DIFaddbegin \DIFadd{. We may define oscillation profiles }\DIFaddend as 
\begin{equation}
...
atom using \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend particular basis set ``BS'' and 
...
we considered CH$_4$, which is \DIFdelbegin \DIFdel{a }\DIFdelend highly symmetric, and therefore, can 
...
energies\DIFaddbegin \DIFadd{~}\DIFaddend \cite{Rothenberg1971,Hariharan1972}. However, to isolate the 
...
$p_{nl}^{\mbox{\scriptsize UGBS}}$ profiles \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend the corresponding 
...
formula to account for the presence of the hydrogens. \DIFaddbegin \DIFadd{This expression 
...
at the origin. 
}\DIFaddend The optimised parameters for the methane molecule are given in 
...
Fig.~\ref{fig:ionch4}. \DIFdelbegin \DIFdel{Experimental data from Ref.~\mbox{%DIFAUXCMD
...
The \DIFdelbegin \DIFdel{Depurated Inversion }\DIFdelend \DIFaddbegin \DIFadd{depurated inversion }\DIFaddend method, on the other hand, accurately 
\DIFdelbegin \DIFdel{reproduce
}\DIFdelend \DIFaddbegin \DIFadd{reproduces }\DIFaddend photoionisation experimental results for many--electron 
atoms. 

We extended the DIM for molecular systems. In this case, the 
inversion procedure \DIFdelbegin \DIFdel{raises huge oscillation }\DIFdelend \DIFaddbegin \DIFadd{produces huge oscillations }\DIFaddend due to the finite size 
...
computed the inverted atomic charges \DIFdelbegin \DIFdel{also }\DIFdelend in a finite--differences 
...
\DIFdelbegin %DIFDELCMD < \bibitem{BatesBook}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem{Bates1962}
\DIFaddend Bates, D. R.; 
...
\DIFaddbegin \bibitem{McDowell1970}
\DIFadd{McDowell, M. R. C.; Coleman, J. P.
}{\it \DIFadd{Introduction to the Theory of Ion--Atom Collisions}}
\DIFadd{North-Holland Publishing Company, Amsterdam, 1970.
}

\DIFaddend \bibitem{Brinkman1930}
